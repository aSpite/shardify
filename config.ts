
export const OPCODES = {
    NFT_TRANSFER: 0x5fcc3d14,
    NFT_OWNERSHIP_ASSIGNED: 0x05138d91,

    EXCESSES: 0xd53276db,

    HOLDER_RETURN: 0x1a3fdbcf,
    HOLDER_RETURN_FAIL: 0xbd017ec7,
    HOLDER_REPEAT_RETURN: 0x6855e245,

    JETTON_DEFRAC_NFT: 0x3a0f1e4c,
    JETTON_INTERNAL_TRANSFER: 0x178d4519,
    JETTON_TRANSFER_NOTIFICATION: 0x7362d09c,
    JETTON_BURN: 0x595f07bc,
    JETTON_BURN_NOTIFICATION: 0x7bdd97de,

    MINTER_CHANGE_PUBKEY: 0x770e2c37,
    MINTER_FRACTIONALIZATION_FEE: 0x363f8bc6,
    MINTER_WITHDRAW_TON: 0x4871ede5,

    POOL_MASTER_CREATE_POOL: 0x198a7cad,
    POOL_MASTER_ADDR_CHANGE_ADMIN: 0x211b3197,
    POOL_MASTER_SIGN_CHANGE_ADMIN: 0xbab1388,
    POOL_MASTER_CHANGE_MINTER_PUBKEY: 0x7d20ca6c,
    POOL_MASTER_WITHDRAW_NFT_MINTER: 0x55fc365f,
    POOL_MASTER_CHANGE_MINTER_CONTENT: 0x1657df13,
    POOL_MASTER_CHANGE_MINTER_ADMIN: 0x2582c414,
    POOL_MASTER_WITHDRAW_TON: 0x541b33c,
    POOL_MASTER_WITHDRAW_TON_MINTER: 0x6e8ad462
}


export const ERRORS = {
    UNAUTHORIZED: 100,
    WRONG_NFT: 101,
    NOT_OWN: 102,
    NOT_NFT_TRANSFER: 103
}